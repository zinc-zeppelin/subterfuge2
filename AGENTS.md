...
5. **Pull Request & Code Review Directives:**
   - **Always Use Pull Requests:** All changes to the codebase must be submitted via a Pull Request against `main` (direct pushes to `main` are strictly prohibited).
   - **Mandatory CodeRabbit Review Pass:** Before merging any PR, all automated review comments and findings generated by CodeRabbit must be fetched, thoroughly reviewed, and addressed.
   - **10-Minute Review Buffer Practice:** Whenever a PR is created or updated, agents and contributors must wait at least **10 minutes** before checking for CodeRabbit comments to ensure the automated analysis has completed without premature polling.
   - **Merge Authorization Policy:**
     - Non-admin contributors and automated agents require at least **one approving review** before merging.
     - Administrators have override authority to force-merge when required.
